<?php
    ...
    function getPerson(){
        $db = dbConnect();
        $req = $db->query("SELECT mail, firstName, lastName, typeAccess, id FROM person ORDER BY typeAccess");
    ...
    function getOrganismIdUser($idUser){
        $db = dbConnect();
        $req = $db->query("SELECT organismId FROM `user` WHERE personId=".$idUser);
    ...
    function getAllLastResults($n){
        $db = dbConnect();
        $req = $db->query("SELECT * FROM results ORDER BY testDate DESC LIMIT ". $n);
    ...
    function getUserAverage($average,$userId){
        $db = dbConnect();
        $req = $db->query("SELECT $average FROM results WHERE userId=".$userId);

        return $req;
    }

    function getAllAverage($userId){
        $db = dbConnect();
        $req = $db->query("SELECT AVG(averageVisualStimulus), AVG(averageSoundStimulus), AVG(averageTone) FROM results WHERE userId=".(int)$userId);
    ...
    function getGlobalAverage(){
        $db = dbConnect();
        $req = $db->query("SELECT AVG(averageVisualStimulus), AVG(averageSoundStimulus), AVG(averageTone) FROM results");

        return $req;
    }

    function getOrganismPersonId($personId){
        $db = dbConnect();
        $req = $db->query("SELECT organismId FROM user WHERE personId=".$personId);
